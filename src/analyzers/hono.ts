...
const MIDDLEWARE_REGEX = /(?:app|hono|router)\.use\s*\(/g;
...
function analyzeHonoFile(
  ...
  for (const call of callExpressions) {
    ...
    const hasHonoValidator = middlewareArgs.some(
      (arg) => HONO_VALIDATOR_REGEX.test(arg.getText()),
...
function getFrameworkChecks(): FrameworkCheck[] {
  return [
    ...
    },
    {
      ...
      description: "POST/PUT/PATCH routes should use zValidator or validator middleware for input validation.",
...
async function checkMissingErrorHandler(
  ...
  return [{
    ...
    description: "No app.onError() found. Add a centralized error handler for production-ready error responses.",
...
async function checkMissing404Handler(
  ...
  return [{
    ...
    description: "No app.notFound() found. Add a custom 404 handler instead of relying on Hono's default.",
    file: files[0].filePath,
    line: null,
    status: "open",
    firstSeen: timestamp,
    ...
    for (const method of route.methods) {
      if (["POST", "PUT", "PATCH"].includes(method.method) && !method.hasValidation) {
        issues.push({
          ...
          description: "This route accepts data but has no validation. Use zValidator() middleware or Hono's built-in validator.",
          file: route.filePath,
...
async function findHonoEntryFiles(projectPath: string): Promise<Array<{ filePath: string; content: string }>> {
